...
if __name__ == '__main__':
    ...
    for i in funIndexes:
        ...
        plotFileName = pathName + "/ABC_F" + str(i+1) + "_" + str(dims) + "D_Plot.csv"
        ...
        bestPlotFileName = pathName + "/ABC_F" + str(i+1) + "_" + str(dims) + "D_BestPlot.csv"
        ...
        worstPlotFileName = pathName + "/ABC_F" + str(i+1) + "_" + str(dims) + "D_WorstPlot.csv"
        ...
        for j in range(numRuns):
            ...
            # Initialization
            ABC = ArtificialBeeColony(functions[i], bounds, popSize=50, workerOnlookerSplit=0.5, limit=None, numScouts=1, optimum=optimums[i]) # F5: -310 / others: -450
            ABC.execute()
            results = ABC.results
            ...
            successes.append( int(results["errors"][-1] <= ABC.tol) )
            ...
            for k in range(len(FESThresholds)):
                ...
                for m in range ( l, len( results["FESCounts"] ) ):
                    ...
                    if(currentFES >= FESThresholds[k] * ABC.maxFES):
